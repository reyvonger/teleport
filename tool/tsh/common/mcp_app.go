...
func (c *mcpConfigCommand) printAutoReconnectHint(w io.Writer) error {
	_, err := fmt.Fprintf(w, `
The --auto-reconnect/--no-auto-reconnect flag controls whether tsh should
automatically starts a new remote MCP session when the previous remote session
is interrupted by network issues or tsh session expirations. Use
--auto-reconnect for better user experience if your MCP sessions are
stateless across requests. If auto-reconnect is disabled, you may need to
restart your client when encountering "disconnected" errors.`)
	return trace.Wrap(err)
}
		...
		return clientmcp.ProxyStdioConnWithAutoReconnect(
			...
			clientmcp.ProxyStdioConnWithAutoReconnectConfig{
				...
				DialServer: func(ctx context.Context) (io.ReadWriteCloser, error) {
					return tc.DialMCPServer(ctx, c.cf.AppName)
				},
				MakeReconnectUserMessage: makeMCPReconnectMessage,
			},
		)
	}
...
func makeMCPReconnectMessage(err error) string {
