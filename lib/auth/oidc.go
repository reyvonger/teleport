...
func (c *oidcClient) startSync(ctx context.Context) {
	...
	go func() {
		c.client.SyncProviderConfig(c.syncCtx, c.connector.GetIssuerURL())
		close(c.firstSync)
		<-c.syncCtx.Done()
	}()
}
...
func (oas *OIDCAuthService) validateOIDCAuthCallback(ctx context.Context, diagCtx *SSODiagContext, q url.Values) (*authclient.OIDCAuthResponse, error) {
	...
	// If a public key was provided, sign it and return a certificate. FIX HERE
...
func (oas *OIDCAuthService) createOIDCUser(p *CreateUserParams, dryRun bool) (types.User, error) {
	...
	if existingUser != nil {
		...
		if _, err := oas.auth.UpdateUser(ctx, user); err != nil {
			return nil, trace.Wrap(err)
		}
	} else {
		if _, err := oas.auth.CreateUser(ctx, user); err != nil {
			return nil, trace.Wrap(err)
		}
	}
